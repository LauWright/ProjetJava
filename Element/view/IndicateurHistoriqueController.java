...
public class IndicateurHistoriqueController {
	...
	/**
	 * construit l'affichage des semaines
	 * 
	 * @param id
	 */
	public void tableSemaine(int id) {
		...
		this.recap = this.gestionDemande(p);
	}

	public String gestionDemande(Programmation pro) {
		this.titrep = "Programmation n° " + pro.getId();
		String recap = "";
		boolean ok = true;
		for (Demande d : this.mainApp.getDemandes()) {
			boolean b = false;
			for (Semaine s : pro.getSemaines()) {
				if (s.getIdSemaine() == d.getIdSemaine()) {
					b = true;
	...
	public void test() {
		...
		/*
		 * for(Programmation p : this.mainApp.getProgrammations()) { for (Semaine s :
		 * p.getSemaines()) { System.out.println(s.getResultat()+"");
		 * 
		 * Set cles = s.getStockPreviEntree().keySet(); Iterator it = cles.iterator();
		 * 
		 * while (it.hasNext()){ Object cle = it.next(); // tu peux typer plus finement
		 * ici Element valeur = s.getStockPreviEntree().get(cle); // tu peux typer plus
		 * finement ici System.out.println(valeur.getNom()); } } }
		 */
	...
	public void creationIndicateurs(int id) {
		...
		for (Node no : this.gridSem.getChildren()) {
			...
			if (GridPane.getRowIndex(no) == i && GridPane.getColumnIndex(no) == 0
					&& no.getClass().getSimpleName().equals("CheckBox")) {
				boolean coche = false;
		...
		for (Semaine s : p.getSemaines()) {
			...
			if (itsOk) {
				...
				semLab.setStyle("-fx-font-weight: bold;");
				...
				while (it.hasNext()) {
					...
					/**
					 * Pour chaque chaine on récupère les entrées et on calcul les dépenses pour la
					 * fabriquer
					 */
					...
					/**
					 * On multiplie le coût d'achat de la production par le nombre de productions
					 * faites
					 */
				...
				/**
				 * S'il n'y a pas de prix de vente on considère que c'est au moins le coût de
				 * production
				 */
				if (venteSemaine <= 0) {
		...
		totalRecap.setStyle("-fx-font-weight: bold;");
	...
	/**
	 * Récupération de l'id de semaine pour une chaine de caractère de ce type
	 * "Semaine id"
	 * 
	...
	public void detailsDemandesShow() {
		boolean okClicked = this.mainApp.showDetailsDemandesDialog(this.recap, this.titrep);
	}
